...
// import { useAuth } from './src/hooks/useAuth'; // Временно отключено для локального тестирования
...
const App: React.FC = () => {
  // const { isAuthenticated, isLoading: authLoading } = useAuth(); // Временно отключено
  const [view, setView] = useState<View>(View.Dashboard);
  // const { checkIns, addCheckIn, importCheckIns, isLoading: checkInsLoading } = useCheckIns(); // Временно отключено
  // const { factors, isLoading: factorsLoading } = useFactors(); // Временно отключено
  
  // Используем локальное хранилище для работы без backend
  const [checkIns, setCheckIns] = useLocalStorage<CheckInRecord[]>('checkInHistory', []);
  const [factors] = useLocalStorage<Factor[]>('factors', INITIAL_FACTORS);
  ...
  // Авторизация временно отключена для локального тестирования без backend
  // Чтобы включить, раскомментируйте импорты useAuth, useCheckIns, useFactors и код ниже:
